...
public class AzureConfigurationUtilities {
    ...
    public static StorageManager getAzureStorageManager(AzureCloudContext azureCloudContext){
          TokenCredential azureCredential = new DefaultAzureCredentialBuilder()
                .tenantId(azureCloudContext.getAzureTenantId())
                .build();

        //Does the storage account exist? If not create it.
        AzureEnvironment azureEnvironment = getAzureEnvironmentByName(azureCloudContext.getAzureEnvironment());

        return StorageManager.authenticate(azureCredential,
                new AzureProfile(azureCloudContext.getAzureTenantId(), azureCloudContext.getAzureSubscriptionId(), azureEnvironment));
