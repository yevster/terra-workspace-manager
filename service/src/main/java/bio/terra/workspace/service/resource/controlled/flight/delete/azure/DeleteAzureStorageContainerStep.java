...
public class DeleteAzureStorageContainerStep implements Step {
    ...
    public StepResult doStep(FlightContext flightContext) throws InterruptedException {
        ...
        final var storageAccount = storageManager.storageAccounts().
                getByResourceGroup(azureContext.getAzureResourceGroupId(),
                        resource.getStorageAccountName());
        final var blobClient = azureContext.authenticatedBlobContainerClientBuilder()
        ...
        if (existingBlobs.iterator().hasNext()) {
            return new StepResult(StepStatus.STEP_RESULT_FAILURE_FATAL, new Exception("Blob Container " + resource.getContainerName() + " is not empty."));
        }

        //Delete the container by name
        final var containerClient = storageManager.serviceClient().getBlobContainers();

        containerClient.delete(azureContext.getAzureResourceGroupId(),
                resource.getStorageAccountName(),
        ...
        var remainingContainers = containerClient.list(
    ...
    public StepResult undoStep(FlightContext flightContext) throws InterruptedException {
        logger.error(
                "Cannot undo delete of Azure Storage Container resource {} in workspace {}.", resourceId, workspaceId);
